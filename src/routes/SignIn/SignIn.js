...
function SignIn() {
  ...
  const handleChange = (event) => {
    ...
    const handleSubmit = (event) => {
      ...
      axios.get('http://localhost/API/login.php', inputs).then(function(response){
  ...
  <form className="flex w-[30rem] flex-col space-y-10" onSubmit={handleSubmit}>
    <div className="text-center text-4xl font-medium text-[#00283f]">Log In</div>
    ...
    <Link to="/SignIn" className="transform text-center font-semibold text-gray-500 duration-300 hover:text-gray-300 text-[#00283f]">FORGOT PASSWORD?</Link>
